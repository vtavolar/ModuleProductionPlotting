...
files = glob.glob('LUT/M*_100*/*XraySpectrum_p17/commander_XraySpectrum.root')
...
def main():
    ...
    for file in files:
        ...
        for chip in range(16):
            ...
            fitResPol2 = FitHistoSpectrum(hpol2, 0., 130.)
            #hpol2.Draw()
            #for fmt in ['.png', '.pdf', '.root']:
                #cplot.SaveAs(str(hpol2.GetName())+'_pol2'+str(fmt))
            sigmapol2 = fitResPol2.GetParameter(4)
            h_deltasigma.Fill(sigmapol2)
    ROOT.gStyle.SetOptStat(111111)
    h_deltasigma.Draw()
    for fmt in ['.png', '.pdf', '.root']:
        cplot.SaveAs('100timesData_deltasigma'+str(fmt))
